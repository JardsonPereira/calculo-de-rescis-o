...
def main():
    ...
    total_prov = res_saldo + res_13 + res_ferias + res_terco + v_aviso_receber + multa_fgts
    ...
    with col_e:
        ...
        st.table({
            "Rubrica": ["Saldo Salário", "13º Proporcional", "Férias Prop.", "1/3 Férias", "Aviso Prévio", "Multa FGTS"],
            "Quantidade": [f"{data_dem.day} dias", f"{avos_13}/12 avos", f"{avos_ferias}/12 avos", "33.3%", f"{dias_totais if aviso_status == 'Indenizado' else dias_exced} dias", "40%/20%"],
            "Valor": [f"{res_saldo:,.2f}", f"{res_13:,.2f}", f"{res_ferias:,.2f}", f"{res_terco:,.2f}", f"{v_aviso_receber:,.2f}", f"{multa_fgts:,.2f}"]
        })
    ...
    with col_d:
        ...
        st.table({
            ...
            "Valor": [f"{desc_inss:,.2f}", f"{desc_faltas:,.2f}", f"{desc_aviso_nao_cump:,.2f}", f"{adiantamento:,.2f}", f"{consignado:,.2f}", f"{estorno_beneficios:,.2f}"]
        })
